...
class ValidationService
{
    ...
    // 驗證密碼格式
    public function validateMobile(?string $country, ?string $mobile): void
    {
    ...
    // 檢查「設定/重設密碼」的輸入是否正確
    public function checkPasswordInputs(?string $password, ?string $confirmed): void
    {
    ...
    // 檢查是否相等
    public function isMatched(string $data_1, string $data_2): bool
    {
        return ($data_1 === $data_2);
    }

    // 檢查「通訊帳號」為電子郵件或手機
    public function checkContactType(Request $request): ?string
    {
        ...
        ($contactType == 'email')
            ? $this->validateEmail($request->$contactType)
            : $this->validateMobile($request->country, $request->$contactType);
